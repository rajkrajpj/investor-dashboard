...
interface RequestItemProps {
  companyName: string;
  details: string;
  onUploadClick: (requestData: { companyName: string; details: string }) => void;
}

const RequestItem: React.FC<RequestItemProps> = ({ companyName, details, onUploadClick }) => {
  return (
    <div className="bg-white shadow-md rounded-lg p-6 flex justify-between items-center">
      ...
      <button 
        onClick={() => onUploadClick({ companyName, details })}
        className="bg-slate-800 hover:bg-slate-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center space-x-2"
      >
      ...
      <div className="space-y-4">
        {requests.map((request, index) => (
          <RequestItem 
            ...
            onUploadClick={() => { /* Placeholder for now, will be implemented in InvestorDashboardUI */ }} 
          />
        ))}
      </div>
    </div>
